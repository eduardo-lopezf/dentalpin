...
- feat(payments): el informe de cobros dice en voz alta la regla que lo gobierna — *«Este informe sigue el cobro, no la facturación»*—, justo donde alguien buscaría la cifra que descarta. La ADR 0010 explica por qué no existe la comparación facturado-vs-cobrado (una clínica deja tratamientos fuera de factura con toda legitimidad, y exponer esa diferencia documenta el operativo), pero quien la buscaba no encontraba ni el dato ni el motivo y concluía que faltaba algo. Una frase convierte un agujero aparente en una decisión.
- feat(payments): **la bandeja recuerda a quién se ha llamado.** Nueva tabla `collection_contacts` (migración `pay_0005`) y dos rutas, `POST/GET /receivables/{patient_id}/contacts`: canal, nota opcional, quién y cuándo. La fila de la bandeja lo enseña —*contactado hoy (WhatsApp)*, o la fecha si es más viejo—, que es lo que evita que dos personas llamen al mismo paciente antes de comer. Cuelga del **paciente** y no de un plan, al revés que el registro del pipeline: lo que se debe puede venir de varios planes y de trabajo que no perteneció a ninguno. **Un contacto no mueve dinero** y esa es su gracia: si el intento funcionó hay un cobro que lo demuestra, y separarlos permite decir «contactado ayer, sigue debiendo 300». El paciente contactado no desaparece de la lista — sigue debiendo, y esconderlo la haría discrepar del total.

- feat(payments): dos fichas para el Resumen de Finanzas (`finance.summary`): **Cobrado** —hoy neto de devoluciones, más el acumulado del mes, resueltos en el día de la clínica y no en el del lector— y **Por cobrar**, con el número de pacientes, cuánto lleva más de 90 días y un botón que entra directo a la bandeja.
- feat(payments): **una bandeja de «Por cobrar», no solo una cifra.** Nueva pestaña en Finanzas junto a *Cobros* y nuevo `GET /receivables` (`payments.record.read`): quién debe, cuánto, desde hace cuántos días y cuándo pagó por última vez, ordenado por deuda más vieja primero, con llamar, WhatsApp y cobro en la fila. El informe ya daba el importe por tramos pero ninguno de los cuatro llevaba a ninguna parte — los cuatro abrían `/patients?with_debt=true`, sin la antigüedad —, así que el número se miraba y de él no salía nada; ahora cada tramo aterriza en la bandeja ya filtrado.
- fix(payments): **la antigüedad de una deuda se medía mal.** `aging_receivables` agrupaba por `min(performed_at)` de *todos* los devengos del paciente, así que uno de tres años al día salvo el empaste de la semana pasada caía en el tramo 90+ junto a la morosidad real, y el tramo que debería avisar de dinero en riesgo se llenaba de los pacientes más fieles. Ahora se mide desde el primer devengo que el dinero no cubrió, recorriendo el FIFO con `coverage_by_earned_entry` —el recorrido sigue teniendo una sola casa—, y el informe lee las mismas filas que la bandeja, así que no pueden discrepar. El seed no lo exhibía (todos sus devengos tienen la misma edad); lo fija `tests/modules/payments/test_receivables.py`.
