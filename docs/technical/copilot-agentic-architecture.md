...
- **Structured tool inputs/results** — key-based redaction over the JSON. The key map is built per redactor from three sources (ADR 0025): the columns the schema classifies with `pii()` (`pii_columns()` — authoritative, and it follows the modules mounted right now), the tenant's jurisdiction document names read from `PrivacyPolicy.jurisdictions` via `Redactor.for_policy()` (ADR 0023 — `curp`/`rfc`/`ine` for `MX`, `dni`/`nif`/`nie`/`cif` for `ES`), and a small set of payload-only keys no column carries (`full_name`, `patient_name`, `mobile`, `telephone`, `phone_number`, `email_address`, `nif`). Plus UUID-valued `*_id` fields known to reference patients/appointments. The same value always maps to the same token within a session (so the model can reason about "the same patient"). A redactor built without a policy falls back to every known jurisdiction, so a forgetful caller over-redacts instead of leaking; an unmodelled country logs a warning and keeps the classified columns. `tests/test_pii_redaction_contract.py` fails when a personal-looking column is left unclassified.
